...
### Companion analysis, wave 12 (32)

32. [`32-reproduction-methods.md`](32-reproduction-methods.md) — **reproduction guide.** Consolidates *every* method used across the series into one step-by-step document so a new analyst can repeat the work without rediscovering the dead ends: the toolchain/environment, **Method A** static unpacking (VMProtect 3.x → Scylla dump on bare metal → Ghidra headless), **Method B** cross-source signature recovery (symbol-bearing Linux build + AGPL BambuStudio headers), **Method C** Linux `.so` dynamic recovery under the three constraints (anti-debug `ptrace`-SIGKILL, static-linked curl/OpenSSL defeating `LD_PRELOAD`, TLS-ciphertext `send`) via `/proc/<pid>/mem` heap-scrape + `dlopen` behavioral oracle + `dlsym`-wrap trace, **Method D** on-wire capture + TLS-key-from-heap decryption, **Method E** config-blob AES-128-ECB interop. Includes a reproduction checklist and an explicit **out-of-scope** section — the Kalay license and the embedded app key are documented as *boundaries*, never extraction targets.
