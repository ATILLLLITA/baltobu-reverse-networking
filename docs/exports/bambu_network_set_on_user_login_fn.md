...
**Group:** callback registration (user_login)
**RVA:** `0x1bc380` &nbsp; **VA:** `0x1801bc380`
**Body size:** 153 bytes &nbsp; **Direct callees:** 1

## Signature

```c
using cb_t = std::function<void(int /* status */, const std::string &/* user_info_json */)>;

int bambu_network_set_on_user_login_fn(agent_t *agent, cb_t *fn);
```
...
Fires when an asynchronous login flow (bambu_network_change_user → credentials posted) completes. status is 0 on success.
