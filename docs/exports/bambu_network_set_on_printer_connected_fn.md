...
**Group:** callback registration (printer_connected)
**RVA:** `0x1bc100` &nbsp; **VA:** `0x1801bc100`
**Body size:** 153 bytes &nbsp; **Direct callees:** 1

## Signature

```c
using cb_t = std::function<void(const std::string &/* dev_id */)>;

int bambu_network_set_on_printer_connected_fn(agent_t *agent, cb_t *fn);
```
...
Fires once a printer transitions to a 'connected' state (either LAN or cloud). Best place to issue device-state queries.
